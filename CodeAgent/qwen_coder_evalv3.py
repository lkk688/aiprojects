...
def mbpp_pass_at_1(
    ...
    """
    MBPP pass@1 with:
      - safer execution (file-based runner, no indentation rewrite)
      - entry-point inference from tests/problem
      - stronger prompt constraints (code-only, correct function name)
      - common stdlib imports prelude
    """
    import re
    import json
    from datasets import load_dataset
    ...
    def infer_entry_point(problem_text: str, tests: List[str]) -> str:
        ...
        # 1) Most reliable: infer from asserts in tests
        # Examples: assert foo(1) == 2
        joined = "\n".join(tests)
        m = re.search(r"assert\s+([A-Za-z_]\w*)\s*\(", joined)
        if m:
            return m.group(1)

        # 2) Next: infer from problem statement phrasing
        # Examples: "Write a function foo" / "Define a function named foo"
    ...
    def build_prompt(problem_text: str, entry_point: str) -> str:
        if entry_point:
            return (
    ...
    for ex in tqdm(ds, desc="MBPP"):
        ...
        prompt1 = build_prompt(problem, entry)
        code_pred = generate_code(prompt1)

        # Ensure tests are executable lines
        test_block = "\n".join(tests)

        full = f"{prelude}{code_pred}\n\n{test_block}\n"
        ok = run_code_subprocess(full, timeout_s=timeout_s)

        if ok:
            passed += 1
            continue

        # Optional second attempt: sometimes model forgets name/signature
        if entry:
            prompt2 = (
                "Return ONLY valid Python code. No markdown fences.\n"
                f"Implement exactly: def {entry}(...):  (use the correct signature from the problem)\n\n"
                f"Problem:\n{problem}\n"
            )
            code_pred2 = generate_code(prompt2)
            full2 = f"{prelude}{code_pred2}\n\n{test_block}\n"
            ok2 = run_code_subprocess(full2, timeout_s=timeout_s)
            if ok2:
                passed += 1
                continue
        ...
        if debug_n and shown < debug_n:
            ...
            print("code_pred_head:", code_pred[:300])
